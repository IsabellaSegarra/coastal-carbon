...
will *simulate* data and perform an analysis on the simulated data. EXPLAIN MORE ABOUT DATA SIMULATION. This includes the following steps:
...
# R computer error: ensure that 0 < fraction_carbon < 1
...
betareg_mdl <- glmmTMB(fraction_carbon ~ fraction_organic_matter + max_depth, 
                      data = blue_carbon, family = beta_family()) # subset data for faster processing 

#betareg_mdl_2 <- glmmTMB(fraction_carbon ~ fraction_organic_matter + max_depth, 
                      #data = blue_carbon[1:10000, ], family = beta_family()) 
...
*Why am I holding `max_depth` constant?* I want to show how `fraction_carbon` changes with one predictor (`fraction_organic_matter`). This isolates the effect of one predictor. 
...
In this section I will calculate the degree of certainty my predictions have using *confidence intervals*. A confidence interval (CI) is a range we are confident contains the population parameter.
```{r}
...
boot <- map_dfr(1:1000, \(i) {
  # boot strap sample
  boot_sample <- slice_sample(blue_carbon, n = 1000, replace = TRUE)
  # fit model 
...
```{r}
#| code-fold: true
# .....Plot predictions..... NEED CONFIDENCE INTERVALS 
predict_plot <- ggplot(betareg_grid, aes(x = fraction_organic_matter, y = predictions)) +
  ...
  geom_point(data = blue_carbon,
             ...
             alpha = 0.5, size = 2) +
  # confidence interval 
  geom_ribbon(data = boot_ci, aes(fraction_organic_matter, ymin = ci_lwr, ymax = ci_upr))
